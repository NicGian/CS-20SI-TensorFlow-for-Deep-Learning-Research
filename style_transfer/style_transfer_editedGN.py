...
import vgg_model_editedGN
import utils

# parameters to manage experiments
STYLE = 'The-Dream-Caused-by-the-Flight-of-a-Bee-1944-Salvador-Dali'#'starry_night'#'guernica'#
CONTENT = 'IMG-20170620-WA0018' #'deadpool'#
...
content_weight = 0.01
style_weight = 1
DIR = '/home/cuda/Desktop/tf_lesson/assignments/style_transfer'
...
def _create_content_loss(p, f):
    ...
    """
    content_loss = tf.reduce_sum((f-p)**2)/(p.size*4.0)
    return content_loss
...
def _gram_matrix(F, N, M):
    ...
    """
    F = tf.reshape(F, (M,N))
    gram_matrix = tf.matmul(tf.transpose(F), F)
    return gram_matrix
...
def _single_style_loss(a, g):
    ...
    single_layer_style_loss = tf.reduce_sum((g_gram-a_gram)**2/((2*N*M)**2))
    return single_layer_style_loss
...
def _create_style_loss(A, model):
    ...
    style_loss = sum([W[i]*E[i] for i in range(n_layers)])
    return style_loss
    ###############################
...
def main():
    ...
    model = vgg_model_editedGN.load_vgg(VGG_MODEL, input_image)
